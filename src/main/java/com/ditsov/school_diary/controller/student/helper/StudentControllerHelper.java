...
public class StudentControllerHelper {
  ...
  public List<StudentResponseBean> listStudents(
      final Optional<Integer> page, final Optional<Integer> size) {
    List<Student> students =
        studentService.getByOrderByFullName(page.orElse(0), size.orElse(10));

    return students
        .stream()
        .map(studentFactory::convertStudentToStudentResponseBean)
