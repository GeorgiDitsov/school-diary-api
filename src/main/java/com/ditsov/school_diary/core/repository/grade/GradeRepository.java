...
public interface GradeRepository extends JpaRepository<Grade, Long> {
  ...
  //          "SELECT AVG(average) "
  //              + "FROM ( "
  //              + "       SELECT grade.student_id, school_course.school_course_id,
  // school_semester.school_semester_id, AVG(grade.value) as average "
  //              + "       FROM grade "
  //              + "       JOIN school_course "
  //              + "       ON grade.school_course_id = school_course.school_course_id "
  //              + "       JOIN school_semester "
  //              + "       ON school_course.school_semester_id = school_semester.school_semester_id
  // "
  //              + "       GROUP BY grade.student_id, school_course.school_course_id,
  // school_semester.school_semester_id "
  //              + "       HAVING grade.student_id = :studentId "
  //              + "       AND school_semester.school_semester_id = :schoolSemesterId"
  //              + "     ) as averages")
  //  Optional<BigDecimal> getSuccessByStudentIdAndSchoolCourseIdAndSchoolSemesterId(
  //      final Long studentId, final Long schoolCourseId, final Long schoolSemesterId);
}
